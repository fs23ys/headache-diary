...
function buildExportText(records: HeadacheRecord[], months: number): string {
  ...
  lines.push('---');
  lines.push('');
  lines.push('## 分析依頼（このまま送信してください）');
  lines.push('');
  lines.push('上記は私個人の頭痛記録データです。このデータをもとに、以下を教えてください。');
  lines.push('');
  lines.push('【必須】以下の3点を必ず含めてください：');
  lines.push('');
  lines.push('1. 個人固有の誘発パターン');
  lines.push('   - 私のデータだけに見られる特徴的なパターンを発見してください');
  lines.push('   - 複数誘因の組み合わせ（例：「睡眠不足×天気変化が重なると発症率○倍」）も分析してください');
  lines.push('   - 具体的な数値・割合・倍率で表現してください');
  lines.push('');
  lines.push('2. 誘因TOP3（このデータに基づくランキング）');
  lines.push('   - 1位・2位・3位を明確に示してください');
  lines.push('   - それぞれ「○○件中○件で発症（○%）」のように根拠を示してください');
  lines.push('');
  lines.push('3. 予防アドバイス（3〜5つ）');
  lines.push('   - このデータから導き出せる、私専用の具体的な予防策を提案してください');
  lines.push('   - 一般論ではなく、このデータに基づいた個別アドバイスにしてください');
  lines.push('   - 実践しやすい行動レベルで書いてください');
  lines.push('');
  lines.push('【補足】余力があれば以下も教えてください：');
  lines.push('- 発症しやすい時間帯・曜日');
  lines.push('- 痛み止めが特に効きやすい・効きにくいパターン');
  lines.push('- 回復を早める要因');
  lines.push('');
  lines.push('※ 医療診断ではなく記録の傾向分析として回答してください。');
